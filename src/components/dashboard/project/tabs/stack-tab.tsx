...
export function StackTab({ project }: { project: Project }) {
    ...
    const handleDownloadPlan = () => {
        if (Object.keys(updates).length === 0) {
            alert("No updates available to generate a plan for.");
            return;
        }

        let md = `# Upgrade Plan for ${project.name}\n`;
        md += `Generated by StackMemory on ${new Date().toLocaleDateString()}\n\n`;

        const vulnerabilityItems: string[] = [];
        const updateItems: string[] = [];
        const allPackages: string[] = [];

        Object.entries(updates).forEach(([pkg, info]) => {
            if (info.vulnerabilities && info.vulnerabilities.length > 0) {
                const vulns = info.vulnerabilities.map((v: any) => v.id).join(', ');
                vulnerabilityItems.push(`- [ ] **${pkg}**: Upgrade to latest (Found: ${vulns})\n  - Command: \`npm install ${pkg}@latest\``);
                allPackages.push(`${pkg}@latest`);
            } else if (info.latest) {
                updateItems.push(`- [ ] **${pkg}**: Upgrade to v${info.latest}\n  - Command: \`npm install ${pkg}@latest\``);
                allPackages.push(`${pkg}@latest`);
            }
        });

        if (vulnerabilityItems.length > 0) {
            md += `## 🚨 Critical Security Fixes\n${vulnerabilityItems.join('\n')}\n\n`;
        }
        ...
        if (allPackages.length > 0) {
            md += `## ⚡ Quick Shell Script\nCopy and paste this into your terminal:\n\n\`\`\`bash\nnpm install ${allPackages.join(' ')}\n\`\`\`\n`;
        } else {
            md += "🎉 All packages are up to date! Nothing to do.\n";
        }
        ...
        a.href = url;
        a.download = `${project.name.toLowerCase().replace(/\s+/g, '-')}-upgrade-plan.md`;
    ...
    const handleDownloadScript = () => {
        ...
        let sh = `#!/bin/bash\n# Upgrade Script for ${project.name}\n# Generated by StackMemory\n\n`;
        sh += `echo "🚀 Starting updates for ${project.name}..."\n\n`;

        const allPackages: string[] = [];
        Object.entries(updates).forEach(([pkg, info]) => {
            if (info.latest) allPackages.push(`${pkg}@latest`);
        });

        if (allPackages.length > 0) {
            sh += `# Installing ${allPackages.length} updates...\n`;
            sh += `npm install ${allPackages.join(' ')}\n\n`;
        } else {
            sh += `echo "No updates found!"\n`;
        }

        sh += `echo "✅ Upgrade complete! Please run tests."\n`;

        const blob = new Blob([sh], { type: 'text/x-sh' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `upgrade-${project.name.toLowerCase().replace(/\s+/g, '-')}.sh`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
                ...
                <div className="flex gap-2">
                    <div className="flex bg-neutral-100 dark:bg-white/5 rounded-full p-1 gap-1">
                        <button
                            ...
                            className="text-xs text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white px-3 py-1.5 rounded-full hover:bg-white/10 transition-colors font-medium flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                            title="Download Markdown Plan"
                        >
                            <FileText className="w-3 h-3" />
                            Plan (.md)
                        </button>
                        <div className="w-px bg-neutral-200 dark:bg-white/10 my-1" />
                        <button
                            onClick={handleDownloadScript}
                            disabled={Object.keys(updates).length === 0}
                            className="text-xs text-neutral-600 dark:text-neutral-400 hover:text-emerald-500 px-3 py-1.5 rounded-full hover:bg-emerald-500/10 transition-colors font-medium flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
